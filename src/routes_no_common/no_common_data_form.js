...
const FORM = require("../models_no_common/no_common_data_form");
...
router.get("/getForConfig", async (req, res, next) => {
  try {
    let { key, status } = req.query;
    let con = [
      {
    ...
    con.push({
      $lookup: {
        from: "no_common_datas",
        localField: "invoice",
        foreignField: "Invoice no",
        as: "no_common_datas"
      }
    })
    const usersQuery = await FORM.aggregate(con);
